...
# --- repo ingestion helpers (already in your repo) ---
...
ENV_SKIP_VIDEO = bool(os.getenv("EXPLAINER_NO_VIDEO", ""))
...
def summarize_file(md_text: str, max_chars: int = 24000) -> str:
    # allow more context by default
...
def pick_research_queries(lang: str, code: str, detail: str = "standard") -> List[str]:
    base = [
    ...
    if detail == "deep":
        base += [
            f"{lang} security hardening",
            f"{lang} deployment strategies",
            f"{lang} observability logging tracing metrics",
        ]
    return list(dict.fromkeys(base))[:10]

# =========================
# LLM explainer (now with detail levels)
# =========================
def openai_explain(
    audience: str,
    ...
    if openai_client is None:
        # offline skeleton
        sfx = " (DEEP DIVE)" if detail == "deep" else (" (BRIEF)" if detail == "brief" else "")
        return {
            "overview": f"(Offline){sfx} OpenAI unavailable: {OPENAI_INIT_ERROR or 'Unknown'}.",
            "key_concepts": [f"Concepts tailored to {audience}{sfx}."],
    ...
    # Build code context (longer if deep)
    joined, total = [], 0
    char_budget = 18000 if detail == "deep" else (12000 if detail == "standard" else 7000)
    for lang, code in code_blocks:
        ...
        if total > char_budget:
            break
        joined.append(piece)
    code_ctx = "".join(joined) if joined else file_summary[:min(char_budget, len(file_summary))]

    depth_msg = {
        "brief": "Be concise. Focus on high-level takeaways and a compact walkthrough.",
        "standard": "Provide clear explanations with practical details and examples.",
        "deep": (
            "Provide a deep-dive with concrete, implementation-level details. "
            "Include architecture, data flow, API surface, testing strategy, performance tuning, "
            "security & compliance considerations, deployment/operations, and a short glossary."
        ),
    }[detail]
    ...
    sys_msg = (
        ...
        "Prefer actionable advice and concrete examples over theory."
    )

    # We keep original schema, but allow optional extended keys.
    user_msg = f"""
{depth_msg}
...
Return a compact JSON with EXACT base keys:
- overview (string)
...
If detail is 'deep', ALSO include OPTIONAL extended keys when helpful:
- architecture (string)
- data_flow (string)
- api_surface (string)
- testing (string)
- security (string)
- deployment (string)
- glossary (array of {{"term": string, "def": string}})
Avoid backticks in values.
"""
    ...
    try:
        resp = openai_client.chat.completions.create(
            ...
            temperature=0.3,
            timeout=90 if detail == "deep" else 60,
        )
    except TypeError:
        resp = openai_client.chat.completions.create(
            model=DEFAULT_MODEL,
            messages=[{"role": "system", "content": sys_msg},
                      {"role": "user", "content": user_msg}],
            temperature=0.3,
            request_timeout=90 if detail == "deep" else 60,
        )
    ...
    # Ensure required keys exist + add optional slots
    defaults = {
        "overview": "",
        "key_concepts": [],
        "walkthrough": "",
...
def _run(cmd: List[str], timeout: int = 360) -> None:
...
def ensure_space_or_raise(min_free_mb=700):
    for p in (PROJECT_TMP, OUTPUT_DIR, VIDEO_DIR):
        _t, _u, free = shutil.disk_usage(p)
...
def tts_to_wav(text: str, wav_path: str, rate_delta: int = 0):
    """
    - On macOS: use 'say' with -f, produce AIFF (default), then ffmpeg -> 16k mono WAV
      (No '--data-format' to avoid 'fmt?' on some macOS builds)
    ...
    chunks = _split_chunks(text, 900 if not FAST_MODE else 700)
...
def ffmpeg_still_with_audio(image_fp: str, audio_fp: str, out_fp: str):
    ...
    try:
        if first == "mp4":
            _run(cmd_mp4, timeout=360)
        else:
            _run(cmd_mov, timeout=360)
            return mov_fp
    except Exception as e:
...
def build_video_from_sections_ffmpeg(
    ...
    ensure_space_or_raise(700 if not FAST_MODE else 400)
...
def make_level_explainer(
    ...
    data = openai_explain(level, code_blocks, summary, detail=detail)

    lang = code_blocks[0][0] if code_blocks else "programming"
    joined_code = "\n".join(cb[1] for cb in code_blocks)[:8000 if detail == "deep" else 5000]
    queries = pick_research_queries(lang, joined_code, detail=detail)

    max_links = 6 if detail == "brief" else (12 if detail == "standard" else 18)
    links: List[str] = []
    for q in queries:
        links.extend(ddg_search_links(q, k=4 if detail == "deep" else 3))
        if len(links) >= max_links:
            break
    def sort_key(u): return (0 if u.startswith("https://") else 1, u)
    links = sorted(list(dict.fromkeys(links)), key=sort_key)[:max_links]

    # Markdown text output
    md_parts = []
    md_parts.append(f"# Code Explainer — {level.capitalize()} ({detail})\n")
    ...
    if data.get("data_flow"):
        md_parts.append("## Data Flow\n" + data["data_flow"].strip() + "\n")
    if data.get("api_surface"):
        md_parts.append("## API Surface\n" + data["api_surface"].strip() + "\n")
    if data.get("testing"):
        md_parts.append("## Testing Strategy\n" + data["testing"].strip() + "\n")
    if data.get("security"):
        md_parts.append("## Security & Compliance\n" + data["security"].strip() + "\n")
    if data.get("deployment"):
        md_parts.append("## Deployment & Operations\n" + data["deployment"].strip() + "\n")
    if data.get("glossary"):
        md_parts.append("## Glossary")
        for entry in data["glossary"]:
            term = str(entry.get("term","")).strip()
            dfn = str(entry.get("def","")).strip()
            if term or dfn:
                md_parts.append(f"- **{term}** — {dfn}")
        md_parts.append("")
    if links:
    ...
    # Narration size per section (longer on deep dive)
    if detail == "brief":
        MAX_PER_SECTION = 280 if FAST_MODE else 420
    elif detail == "deep":
        MAX_PER_SECTION = 700 if FAST_MODE else 1100
    else:
        MAX_PER_SECTION = 420 if FAST_MODE else 900
    ...
    # Build slide sections (include deep-dive parts if present)
    sections = []
    if data.get("overview"):
        sections.append(("Overview", short(data["overview"])))
    if data.get("key_concepts"):
        sections.append(("Key Concepts", short("; ".join(data["key_concepts"][:8]))))
    if data.get("walkthrough"):
        sections.append(("Walkthrough", short(data["walkthrough"])))
    if data.get("complexity"):
        sections.append(("Complexity", short(data["complexity"])))
    if data.get("pitfalls"):
        sections.append(("Pitfalls", short("; ".join(data["pitfalls"][:8]))))
    # deep extras
    if detail == "deep":
        for (tkey, ttitle) in [
            ...
            if data.get(tkey):
                sections.append((ttitle, short(data[tkey])))
    if data.get("tl_dr"):
        sections.append(("TL;DR", short(data["tl_dr"])))

    video_path = None
    # Respect per-run toggle; env is just a default
    if include_video and sections:
        try:
            ensure_space_or_raise(700 if detail == "deep" else 500)
...
def pipeline(file_obj, levels: List[str], detail_choice: str, want_video: bool):
    if file_obj is None:
    ...
    detail = {"Brief": "brief", "Standard": "standard", "Deep-dive": "deep"}.get(detail_choice, "standard")
    include_video = (not ENV_SKIP_VIDEO) and bool(want_video)

    results: Dict[str, ExplainerResult] = {}
    for lvl in levels:
        results[lvl] = make_level_explainer(lvl, md_text, blocks, detail=detail, include_video=include_video)
...
# =========================
# GitHub & Local pipelines
# =========================
def pipeline_from_github(repo_url: str, levels: List[str], ref: str, token: str, detail_choice: str, want_video: bool):
    ...
    return pipeline(_Tmp, levels, detail_choice, want_video)

def pipeline_from_local(local_path: str, levels: List[str], detail_choice: str, want_video: bool):
...
# Gradio UI
# =========================
with gr.Blocks(title="Code Explainer (Markdown/Repo → Text + Video)") as demo:
    gr.Markdown(
        """
# 🧠 Code Explainer — Markdown / GitHub / Local → Text + Video
Upload a **code markdown** file OR point at a **GitHub repo** / **local folder**.  
Builds audience-specific explanations (Beginner / Intermediate / Advanced), fetches **reference links**, and (optionally) creates **local videos** with offline/OS TTS.
...
**Fast mode:** set `EXPLAINER_FAST=1` for smaller, faster videos
    ...
    with gr.Tabs():
        with gr.Tab("Upload .md"):
            ...
            with gr.Row():
                detail_u = gr.Radio(
                    choices=["Brief", "Standard", "Deep-dive"],
                    value="Standard",
                    label="Detail level"
                )
                want_video_u = gr.Checkbox(value=not ENV_SKIP_VIDEO, label="Create video?")
        ...
        with gr.Tab("GitHub Repo"):
            ...
            with gr.Row():
                ...
                want_video_g = gr.Checkbox(value=not ENV_SKIP_VIDEO, label="Create video?")
        ...
        with gr.Tab("Local Folder"):
            ...
            with gr.Row():
                ...
                want_video_l = gr.Checkbox(value=not ENV_SKIP_VIDEO, label="Create video?")
    ...
    run_btn_u.click(
        fn=pipeline,
        inputs=[file_in, levels_u, detail_u, want_video_u],
        outputs=[text_out, vids, files, save_loc],
        api_name="run_from_md"
    )
    run_btn_g.click(
        fn=pipeline_from_github,
        inputs=[gh_url, levels_g, gh_ref, gh_token, detail_g, want_video_g],
        outputs=[text_out, vids, files, save_loc],
        api_name="run_from_github"
    )
    run_btn_l.click(
        fn=pipeline_from_local,
        inputs=[local_path, levels_l, detail_l, want_video_l],
        outputs=[text_out, vids, files, save_loc],
        api_name="run_from_local"
    )
