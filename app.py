...
# def _purge_old_tmp(prefixes=("cx_", "cxconcat_", "tmp_cx_"), max_age_hours=6):
#     now = time.time()
#     for name in os.listdir(PROJECT_TMP):
#         if any(name.startswith(p) for p in prefixes):
#             p = os.path.join(PROJECT_TMP, name)
...
# def ensure_space_or_raise(min_free_mb=300):
#     for p in (PROJECT_TMP, OUTPUT_DIR, VIDEO_DIR):
#         total, used, free = shutil.disk_usage(p)
#         if free < min_free_mb * 1024 * 1024:
#             raise RuntimeError(f"Low disk space (<{min_free_mb}MB) in {p}")
...
#                 subprocess.run(cmd, check=True, timeout=120)
#             except Exception as e:
#                 print("[proc] ERROR rc= 1")
#                 print("[proc] stderr tail:\n Opening output file failed: fmt?")
#                 # fall back to pyttsx3 for entire text
#                 use_say = False
#                 break
#             finally:
#                 try:
...
#     if not use_say:
#         # Rebuild as a single AIFF via pyttsx3
#         aiff_fp = os.path.join(work_dir, f"{base}_seg1.aiff")
#         engine = pyttsx3.init()
#         rate = engine.getProperty("rate")
#         engine.setProperty("rate", max(100, rate + rate_delta))
...
#     # If multiple AIFF segments, concat them into one AIFF via ffmpeg
#     if len(aiff_segments) == 1:
#         concat_aiff = aiff_segments[0]
#     else:
#         tmpdir = tempfile.mkdtemp(prefix="tmp_cx_")
...
# def ffmpeg_still_with_audio(image_fp: str, audio_fp: str, out_fp: str):
#     """
#     Try MP4 first; if it fails, try MOV (MJPEG+PCM); if that fails, retry MP4 with safer flags.
#     """
#     ensure_space_or_raise(200)
...
#     # Decide preferred first attempt based on env
#     first = "mp4" if PREF_CONTAINER == "mp4" else ("mov" if PREF_CONTAINER == "mov" else "mp4")

#     try:
#         if first == "mp4":
#             _run(cmd_mp4, timeout=240)
#         else:
#             _run(cmd_mov, timeout=240)
#             return mov_fp
#     except Exception as e:
#         print("[ffmpeg] primary attempt failed, retrying alternate:", e)
...
#         except Exception as e2:
#             print("[ffmpeg] alternate attempt failed:", e2)
#             # As a last resort, try a minimal MP4 without loop flag
#             cmd_safe = [
#                 FFMPEG_BIN, "-y", "-hide_banner", "-loglevel", "error", "-nostdin",
#                 "-f", "image2", "-framerate", "1", "-i", image_fp,
#                 "-analyzeduration", "16k", "-probesize", "16k",
...
#         """
# # 🧠 Code Explainer — Markdown → Text + Video (Local TTS)
# Upload a **code markdown** file. This builds audience-specific explanations (Beginner / Intermediate / Advanced), fetches **reference links**, and creates **local videos** with offline/OS TTS.
...
#     run_btn = gr.Button("Generate Explanations + Videos", variant="primary")
